...
const ProductItem = ({ product }: { product: ProductItem }) => {
  ...
  return (
    <Link href={`/product/${product.id}`}>
      <div className="w-48 lg:w-56 min-w-full flex flex-col gap-2 relative bg-white p-3 hover:drop-shadow-front hover:z-10 transition-all">
        <Image className="self-center" src={image} alt="" layout="responsive" />
        <h2 className="text-base h-12 text-gray-500 line-clamp-2">{name} </h2>
        <div className="flex flex-row justify-between items-center">
          <Chip text={`${discount}%`} />
          <div className="flex flex-col items-end ">
            <h3 className="text-sm text-gray-400 justify-between line-through">
